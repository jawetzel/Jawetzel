/**
 * Supply-search tuning — pure domain constants for the embroidery-supplies
 * color search. One source of truth for every caller (the supplies page, the
 * supply API route, the `find_thread_color` AI tool, and the assistant system
 * prompt). Zero I/O — lives in the domain so the chat use-case can depend on it
 * without reaching into `@/lib`.
 *
 * The neighbor gap between swatches in the "Explore nearby" row is derived —
 * always `2 * tolerance` — so adjacent neighborhoods never share a match. No
 * separate constant is needed.
 */
